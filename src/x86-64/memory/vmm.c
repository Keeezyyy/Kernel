...
static inline int get_empty_slot_in_table(uint64_t *table, uint16_t start_index,
                                          uint16_t end_index) {
...
static void parse_pointers_from_indices(parsed_virtual_address indices,
                                        uint64_t **out_pml4,
                                        uint64_t **out_pdpt, uint64_t **out_pd,
                                        uint64_t **out_pt) {
...
static inline bool is_entry_huge(uint64_t entry) { return entry & PTE_PS; }
//------------------------------------------------------------------------------------------------------------------------------------------------------

// Init
void vmm_init() {
  // memory.h util function to parse table indices from va
  const parsed_virtual_address indices =
      parse_virtal_address(vmm_get_new_page_table_offset());

  uint64_t *pml4_table = (uint64_t *)get_va_from_physical_address(get_cr3());
  uint64_t *pdpt = (uint64_t *)get_va_from_physical_address(
      pml4_table[indices.pml4_index] & 0x000FFFFFFFFFF000ULL);
  ...
  for (int i = 0; i < MAX_RAM_IN_GB; i++) {
    ...
    pdpt[i] = PTE_GET_ADDR(i * GIGABYTE) | PTE_PRESENT | PTE_RW | PTE_PS;
  }
...
uint64_t vmm_get_new_page_table_offset() { return DIRECT_MAPPING_OFFSET; }
//------------------------------------------------------------------------------------------
...
static int find_contigouos_slot_in_table(uint64_t *table, uint16_t size,
...
find_contigouos_slot(const uint64_t const *pml4, const struct vm_area *area,
                     uint32_t size) {
  ...
  const parsed_virtual_address ending_indices =
      parse_virtal_address(area->end_address);

  // TODO: support larger pages
  for (int i = starting_indices.pml4_index; i < ending_indices.pml4_index;
       i++) {
...
static void set_empty_pdpt_into_memory(const parsed_virtual_address indices,
                                       uint64_t *pml4) {
  // new pdpt, pd, pt
...
alloc_new_mapping_slot(uint64_t *pml4, const struct vm_area *area,
  ...
  for (uint16_t i = starting_index; i <= ending_index; i++) {
    ...
    if (!is_entry_present(pml4[i])) {
      parsed_virtual_address idx = {
          .pml4_index = i, .pdpt_index = 0, .pd_index = 0, .pt_index = 0};
    ...
    for (uint16_t k = 0; k < 512; k++) {
      ...
      if (!is_entry_present(pdpt_e)) {
        parsed_virtual_address idx = {
            .pml4_index = i, .pdpt_index = k, .pd_index = 0, .pt_index = 0};
      ...
      for (uint16_t j = 0; j < 512; j++) {
        ...
        if (!is_entry_present(pd_e)) {
          parsed_virtual_address idx = {
              .pml4_index = i, .pdpt_index = k, .pd_index = j, .pt_index = 0};
...
static void finilize_new_mapping(const parsed_virtual_address indices,
                                 uint32_t length) {
...
static parsed_virtual_address get_free_list_slot(uint64_t *pml4) {
  ...
  const parsed_virtual_address starting_indices =
      parse_virtal_address(START_FREE_LIST);
  const parsed_virtual_address ending_indices =
...
static const parsed_virtual_address
alloc_new_free_list_mapping_slot(uint64_t *pml4) {
  const parsed_virtual_address starting_indices =
      parse_virtal_address(START_FREE_LIST);
  const parsed_virtual_address ending_indices =
      parse_virtal_address(END_FREE_LIST);
...
int vmm_free(void *va) {
  ...
  // TODO:: make proper error managment
  return -1;
}

// Allocation
void *vmm_alloc(uint32_t size, enum virtual_mem_area_enum type,
                int *ERR) // Size in pages (4096 Bytes)
{
  uint64_t *pml4 = get_pml4();
