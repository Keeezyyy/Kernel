...
void init_gdt(void) {
  ...
  gdt_entries[0] = (gdt_entry_t){.limit_low = 0, .base_low = 0, .base_middle = 0, .access = 0, .granularity = 0, .base_high = 0};

  gdt_entries[1] = (gdt_entry_t){.limit_low = 0x0000, .base_low = 0x0000, .base_middle = 0x00, .access = GDT_KERNEL_CODE_ACCESS, .granularity = GDT_KERNEL_CODE_FLAGS, .base_high = 0x00};

  gdt_entries[2] = (gdt_entry_t){.limit_low = 0x0000, .base_low = 0x0000, .base_middle = 0x00, .access = GDT_KERNEL_DATA_ACCESS, .granularity = GDT_KERNEL_DATA_FLAGS, .base_high = 0x00};

  gdt_entries[3] = (gdt_entry_t){.limit_low = 0x0000, .base_low = 0x0000, .base_middle = 0x00, .access = GDT_USER_CODE_ACCESS, .granularity = GDT_USER_CODE_FLAGS, .base_high = 0x00};

  gdt_entries[4] = (gdt_entry_t){.limit_low = 0x0000, .base_low = 0x0000, .base_middle = 0x00, .access = GDT_USER_DATA_ACCESS, .granularity = GDT_USER_DATA_FLAGS, .base_high = 0x00};

  gdt_entries[5] = (gdt_entry_t){.limit_low = tss_limit & 0xFFFF,
  ...
  gdt_entries[6] = (gdt_entry_t){.limit_low = (tss_base >> 32) & 0xFFFF, .base_low = (tss_base >> 48) & 0xFFFF, .base_middle = 0, .access = 0, .granularity = 0, .base_high = 0};
  ...
  debug_printf("[GDT] : GDT Setup Complete\n");
}
